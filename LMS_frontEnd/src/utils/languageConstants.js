const lang = {
  en: {
    //Header
    header: "R/Pathagama Maha Vidyalaya",
    home: "Home",
    about: "About",
    academics: "Academics",
    lms: "LMS",
    notice: "Notice",
    gallery: "Gallery",
    contact: "Contact Us",

    // HeroImageCarousel
    tagline:
      "where we nurture young minds and guide them toward a brighter future with our dedicated faculty and excellent facilities.",
    scrollDown: "Scroll Down",

    //AboutSection
    about_title: "55 Years of Excellence",
    about_description:
      "Welcome to R/Pathagama Maha Vidyalaya, where we provide a nurturing environment for students to learn, grow, and excel. Our dedicated faculty and state-of-the-art facilities ensure a well-rounded education that prepares students for success in the modern world.",
    about_students: "Students",
    about_teachers: "Teachers",
    about_classes: "Classes",

    //SchAbout
    tagline_2:
      "Established in 1891, we have been shaping futures with a perfect blend of tradition and innovation.",
    mission:
      "To provide a nurturing environment that fosters academic excellence, personal growth, and social responsibility, preparing students to thrive in a rapidly changing world.",
    features: [
      {
        title: "130+ Years",
        description: "Of academic excellence and tradition",
      },
      {
        title: "600+ Students",
        description: "Nurturing young minds every year",
      },
      { title: "30+ Faculty", description: "Dedicated and experienced staff" },
      {
        title: "Modern Curriculum",
        description: "Balancing tradition and innovation",
      },
    ],

    // Body
    eLearningHeader: "e-Learning",
    eLearningDescription:
      "Our Learning Management System transforms e-learning into an immersive experience. Engage in interactive sessions, collaborate seamlessly, and access world-class resources all on one innovative platform.",
    exploreLMS: "Explore LMS",

    // Footer
    copyright: "© 2024 R/Pathagama Maha Vidyalaya, All Rights Reserved.",

    //About
    missionTitle: "Mission",
    missionText:
      "Ensuring a culturally diverse environment for students from all over the country, to enable them to become active learners and well-rounded citizens who contribute positively to the world.",
    visionTitle: "Vision",
    visionText:
      "Providing each and every student with a unique learning environment to grow and excel from the time they begin their educational journey while offering a vibrant and enjoyable educational opportunity to face lifelong challenges.",
    historyTitle: "History",
    historySubtitle: "Established 1920",
    historyText:
      "R/Pathagama Maha Vidyalaya, a cornerstone of education in the region, was established with the mission of providing quality education to the children of Pathagama and its surrounding areas. The school was founded in 1920, under the guidance of visionary educators and community leaders who recognized the need for accessible and inclusive education.",
    historyQuote:
      '"From humble beginnings to academic excellence - shaping futures for over a century"',
    leadershipTitle: "Leadership Through the Years",
  },
  sinhala: {
    //Header
    header: "ර/පාතගම මහා විද්‍යාලය",
    home: "මුල් පිටුව",
    about: "අපි ගැන",
    academics: "අධ්‍යාපනය",
    lms: "LMS",
    notice: "දැන්වීම්",
    gallery: "ගැලරිය",
    contact: "අමතන්න",

    // HeroImageCarousel
    tagline:
      "එහිදී අපි අපගේ කැපවූ පීඨය සහ විශිෂ්ට පහසුකම් සමඟින් තරුණ මනස පෝෂණය කර දීප්තිමත් අනාගතයක් කරා ඔවුන්ව මෙහෙයවන්නෙමු.",
    scrollDown: "පහළට යන්න.",

    //AboutSection
    about_title: "අවසරයි 55 වසරක්",
    about_description:
      "ර/පාතගම මහා විද්‍යාලය වෙත සාදරයෙන් පිළිගනිමු, අපි සිසුන්ට ඉගෙනීමට, වැඩිදියුණු වීමට සහ විශිෂ්ටත්වය ලබා ගැනීමට පරිසරයක් ලබා දෙනවා. අපගේ කැපවූ පංති කණ්ඩායම සහ නවීන පහසුකම් සිසුන්ට නවීන ලෝකයට සුදානම් වීමට උපකාරී වේ.",
    about_students: "සිසුන්",
    about_teachers: "ගුරුවරුන්",
    about_classes: "පන්ති",

    //SchAbout
    tagline_2:
      "1891 වසරේ ආරම්භක වූ අපි, සම්ප්‍රදාය සහ නව්‍යකරණය එකට මුසු කරමින් ගමන් කරමි.",
    mission:
      "අධ්‍යාපනීය විශිෂ්ටත්වය, පුද්ගලික වර්ධනය සහ සමාජීය වගකීම් වර්ධනය කරන, ශිෂ්‍යයින් සැලසුම් සහිතව වර්ධනය වන ලෝකයක ජයගැනීමට උපකාරී පරිසරයක් සැලසීම.",
    features: [
      {
        title: "130+ වසර",
        description: "අධ්‍යාපනික විශිෂ්ටත්වය සහ සම්ප්‍රදාය",
      },
      {
        title: "600+ ශිෂ්‍යයින්",
        description: "සෑම වසරකම තරුණ මනසක් පෝෂණය කරයි",
      },
      {
        title: "30+ ගුරු මණ්ඩලය",
        description: "අදාල හා අත්දැකීම් ඇති ගුරුවරු",
      },
      {
        title: "නවීන විෂය නිර්දේශය",
        description: "සම්ප්‍රදාය හා නව්‍යකරණය අතර සමබරතාවය",
      },
    ],

    //Body
    eLearningHeader: "ඊ-අධ්‍යාපනය",
    eLearningDescription:
      "අපගේ ඉගෙනීමේ කළමනාකරණ පද්ධතිය ඊ-අධ්‍යාපනය අතහැරීමකට පත් කිරීමට හැකියාවක් ලබා දෙයි. අන්තර්ක්‍රියාකාරී සැසිවල නිරත වන්න, අභ්‍යන්තරව එකට කටයුතු කරන්න, සහ ලෝක ප්‍රමුඛ සම්පත් ප්‍රවේශමක් ලබා ගන්න.",
    exploreLMS: "LMS ගවේෂණය කරන්න",

    //Footer
    copyright: "© 2024 ර/පාතගම මහා විද්‍යාලය, සියලුම හිමිකම් ඇවිරිණි.",

    //About
    missionTitle: "මූලධර්මය",
    missionText:
      "රටේ සෑම කොටසකින්ම ශිෂ්‍යයන් සඳහා සංස්කෘතික විවිධත්වය පවතින පරිසරයක් සැලසීමෙන්, ඔවුන්ව ක්‍රියාකාරී ඉගෙනුම්කරුවන් සහ යහපත් පුරවැසියන් බවට පත්කරනු ඇත.",
    visionTitle: "දැක්ම",
    visionText:
      "සෑම ශිෂ්‍යයකුටම ඔවුන්ගේ අධ්‍යාපන ගමන ආරම්භ කරන අවස්ථාවෙන්ම වර්ධනය වන සහ උත්සාහය දක්වන සුවිශේෂී ඉගෙනුම් පරිසරයක් සැලසීම.",
    historyTitle: "ඉතිහාසය",
    historySubtitle: "1920දී ආරම්භ කරන ලදී",
    historyText:
      "ර/පාතගම මහා විද්‍යාලය, පාසල ආරම්භ කරන ලද්දේ පාතගම සහ එහි අවට ප්‍රදේශවල ළමයින්ට ගුණාත්මක අධ්‍යාපනය ලබාදීමේ මෙහෙවරක් ලෙසයි. 1920දී ආරම්භ වූ පාසලේ පුරෝගාමී ගුරුවරුන් සහ ප්‍රජා නායකයින් අධ්‍යාපනය ප්‍රවේශමය සහ සමානාත්මතාවය සලකා ආරම්භ කරන ලදී.",
    historyQuote:
      '"කුඩා ආරම්භයකින් විශිෂ්ටත්වයට - සියවසක් පුරා අනාගතයන් සැලසුම් කරයි"',
    leadershipTitle: "වසර තුළ නායකත්වය",
  },
  tamil: {
    //Header
    header: "ர/பதாகம மகா வித்தியாலயம்",
    home: "முகப்பு",
    about: "எங்களை பற்றி",
    academics: "கல்வி",
    lms: "LMS",
    notice: "விளம்பரங்கள்",
    gallery: "கேலரி",
    contact: "தொடர்பு கொள்ள",

    // HeroImageCarousel
    tagline:
      "அங்கு நாங்கள் இளம் மனங்களை வளர்த்து, எங்கள் அர்ப்பணிப்புள்ள ஆசிரியர்கள் மற்றும் சிறந்த வசதிகளுடன் அவர்களை பிரகாசமான எதிர்காலத்தை நோக்கி வழிநடத்துகிறோம்.",
    scrollDown: "கீழே உருட்டவும்",

    //AboutSection
    about_title: "55 வருட சிறப்பு",
    about_description:
      "ஆர்/பதாகம மகா வித்யாலயாவிற்கு வருக, அங்கு மாணவர்கள் கற்கவும், வளரவும், சிறந்து விளங்கவும் ஒரு வளர்ப்பு சூழலை நாங்கள் வழங்குகிறோம். எங்கள் அர்ப்பணிப்புள்ள ஆசிரியர்களும் அதிநவீன வசதிகளும் நவீன உலகில் மாணவர்களை வெற்றிக்குத் தயார்படுத்தும் ஒரு நன்கு வட்டமான கல்வியை உறுதி செய்கின்றன.",
    about_students: "மாணவர்கள்",
    about_teachers: "ஆசிரியர்கள்",
    about_classes: "வகுப்புகள்",

    //SchAbout
    tagline_2:
      "1891-ல் நிறுவப்பட்டது, பாரம்பரியம் மற்றும் புதுமையை இணைத்து எதிர்காலங்களை உருவாக்குகிறோம்.",
    mission:
      "அறிவுசார் மேம்பாடு, தனிநபர் வளர்ச்சி மற்றும் சமூகப் பொறுப்பு ஆகியவற்றை வளர்க்கும் உதவியான சூழலாக மாற்றி மாணவர்களை மாற்றம் அடையும் உலகில் தழைத்தோங்க தயாராக்குவது.",
    features: [
      {
        title: "130+ ஆண்டுகள்",
        description: "கல்விசார் சிறப்பு மற்றும் பாரம்பரியம்",
      },
      {
        title: "600+ மாணவர்கள்",
        description: "ஒவ்வொரு ஆண்டும் இளம் மனத்களை வளர்க்கிறோம்",
      },
      {
        title: "30+ ஆசிரியர்கள்",
        description: "அனுபவம் வாய்ந்த மற்றும் அர்ப்பணிப்புள்ள ஆசிரியர்கள்",
      },
      {
        title: "நவீன பாடத்திட்டம்",
        description: "பாரம்பரியத்திற்கும் புதுமைக்கும் சமநிலை",
      },
    ],

    //Body
    eLearningHeader: "எட்-ஆத்ராஜ்",
    eLearningDescription:
      "நம்முடைய கல்வி மேலாண்மை அமைப்பு ஈட்-ஆத்ராஜ் கைவிடப்படுவதற்கு வாய்ப்பு கிடைக்கும். இடையிடையே செயல்படும் செயலிகளில் ஈடுபடுங்கள் ஒன்றாகச் செயல்படுங்கள், மற்றும் உலகப் பிரசித்தி பெற்ற வளங்கள் அணுகலைப் பெறுங்கள்.",
    exploreLMS: "LMS தேடு",

    //Footer
    copyright:
      "© 2024 ர/பதாகம மகா வித்தியாலயம், அனைத்து உரிமைகளும் பாதுகாக்கப்பட்டுள்ளன.",

    //About
    missionTitle: "பணி",
    missionText:
      "நாடு முழுவதிலுமிருந்து வரும் மாணவர்களுக்கு கலாச்சார ரீதியாக பன்முகத்தன்மை கொண்ட சூழலை உறுதி செய்தல், அவர்கள் சுறுசுறுப்பான கற்பவர்களாகவும், உலகிற்கு நேர்மறையான பங்களிப்பை வழங்கும் நன்கு வளர்ந்த குடிமக்களாகவும் மாறுவதற்கு.",
    visionTitle: "பார்வை",
    visionText:
      "ஒவ்வொரு மாணவரும் தங்கள் கல்விப் பயணத்தைத் தொடங்கும் நேரத்திலிருந்தே வளரவும் சிறந்து விளங்கவும் ஒரு தனித்துவமான கற்றல் சூழலை வழங்குதல், அதே நேரத்தில் வாழ்நாள் முழுவதும் சவால்களை எதிர்கொள்ள ஒரு துடிப்பான மற்றும் மகிழ்ச்சிகரமான கல்வி வாய்ப்பை வழங்குதல்.",
    historyTitle: "வரலாறு",
    historySubtitle: "1920 இல் நிறுவப்பட்டது",
    historyText:
      "பதகம மற்றும் அதன் சுற்றியுள்ள பகுதிகளின் குழந்தைகளுக்கு தரமான கல்வியை வழங்கும் நோக்கத்துடன், இப்பகுதியில் கல்வியின் மூலக்கல்லான ஆர்/பதகம மகா வித்யாலயா நிறுவப்பட்டது. அணுகக்கூடிய மற்றும் உள்ளடக்கிய கல்வியின் அவசியத்தை உணர்ந்த தொலைநோக்கு கல்வியாளர்கள் மற்றும் சமூகத் தலைவர்களின் வழிகாட்டுதலின் கீழ், இந்தப் பள்ளி 1920 இல் நிறுவப்பட்டது.",
      historyQuote:
      '"தாழ்மையான தொடக்கத்திலிருந்து கல்விச் சிறப்பிற்கு - ஒரு நூற்றாண்டுக்கும் மேலாக எதிர்காலத்தை வடிவமைத்தல்"',
      leadershipTitle: "ஆண்டுகள் முழுவதும் தலைமைத்துவம்",
  },
};

export default lang;
